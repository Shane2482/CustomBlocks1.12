...
public class test extends Block implements IMetaBlockName {

	public static final PropertyEnum TYPE = PropertyEnum.create("type", lampType.class);

	public final boolean isOn;

	public test(String name, Material material, boolean isOn) {
		super(material);
...
//		for (int i = 0; i < lampType.values().length; i++) {
//			CustomBlocks.proxy.addRenderRegister(new ItemStack(this, 1, i), getRegistryName(),
...
//	}
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, ItemStack heldItem, EnumFacing side, float hitX, float hitY, float hitZ) {
		if (!world.isRemote && !player.isSneaking()) {
			world.setBlockState(pos, (this.isOn ? BlockInit.Lamps : BlockInit.LampsOn).getStateFromMeta(this.getMetaFromState(state)), 2);
	...
	private void updateLamps(World world, BlockPos pos, boolean powered, List<BlockPos> list) {
		...
		if (block instanceof test) {
			boolean isOn = ((test) block).isOn;
			int meta = block.getMetaFromState(state);
			if (powered) {
				if (!isOn) {
					world.setBlockState(pos, BlockInit.LampsOn.getStateFromMeta(meta), 2);
				}
			} else {
				if (isOn) {
					world.setBlockState(pos, BlockInit.Lamps.getStateFromMeta(meta), 2);
				}
			}
